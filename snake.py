...
class Snake:
    def __init__(self, snake_size, canvas_size):
        ...
        for i in range(self.snake_size, 0, -1):
            start_pos = 20 * i - 10
            self.pieces.append(self.Segment(posx=start_pos, posy=10))
    ...
    class Segment(Turtle):
        ...
        def cross_margin(self, limit):
            ...
            if posx > max_x:
                self.goto(min_x, posy)
            # crossing the left side
            elif posx < min_x:
                self.goto(max_x, posy)
            # crossing the upper side
            elif posy > max_y:
                self.goto(posx, min_y)
            # crossing the bottom side
            elif posy < min_y:
                self.goto(posx, max_y)
    ...
    def add_tail(self):
        ...
        self.pieces.append(self.Segment(new_x, new_y))
    ...
    # apple as argument to know its position
    def move_snake(self, apple):
        head = self.pieces[0]
        for piece in self.pieces:
            # check if the head eats the apple
            # GREATEST BUG AHEAD. After some movements, position got decimal places
            # and was not equal to apple position, so eaten was not changed to true
            head_position = (round(head.pos()[0]), round(head.pos()[1]))
            if piece == head and head_position == apple.position():
                self.add_tail()
                self.eaten = True
            piece.move_segment()
            piece.cross_margin(self.limit)

    def return_position(self):
        print(f'Head: {[x for x in self.pieces[0].position()]}')
